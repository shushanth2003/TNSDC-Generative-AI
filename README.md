...
## Model Evalution
### The process begins when a user submits a query, which can be anything from a short text to a longer document. This input is then divided into smaller parts called "Chunks" for easier handling. Each chunk is then transformed into a numerical form using a technique called word embedding, which helps capture the meaning of words in a numerical way. These numerical representations are stored in a database called FAISS (Facebook AI Similarity Search), designed to quickly find similar items in large datasets. When a user submits a query, the system looks for similar documents in the database based on these numerical representations. These similar documents are then presented to the user as "Matched Docs." This simple process allows users to find relevant documents quickly and easily.

![Screenshot 2024-03-28 220128](https://github.com/shushanth2003/TNSDC-Generative-AI/assets/103485945/273bdafb-37eb-4b5e-9feb-d1b482fd463a)
